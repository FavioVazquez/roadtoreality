...
(function () {
  ...
  var boxX = W * 0.42, boxW = W * 0.28, boxH = H * 0.34, boxY = H * 0.42;
  var PERIOD = 6;

  /* Pie chart layout — right side panel */
  var pieCX = W * 0.80;
  var pieCY = H * 0.32;
  var pieR  = Math.min(W, H) * 0.10;

  /* Energy readout positioned BELOW pie chart */
  var readY = pieCY + pieR + 20;
  ...
  function drawFrame() {
    ...
    /* Pulley */
    ctx.beginPath();
    ctx.arc(W * 0.22, weightY0 + 10, 12, 0, Math.PI * 2);
    ...
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(W * 0.22, weightY0 + 22);
    ctx.lineTo(W * 0.22, weightY);
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(W * 0.22 + 12, weightY0 + 10);
    ...
    ctx.fillRect(W * 0.22 - 18, weightY, 36, 24);
    ctx.strokeStyle = COLOR;
    ctx.lineWidth = 1.5;
    ctx.strokeRect(W * 0.22 - 18, weightY, 36, 24);

    /* "Arrived" label — positioned 28px below bottom of weight box,
       giving ≥20px clearance from the water container top label */
    var arrivedLabelY = weightYMax + 24 + 28;
    if (cycle > 0.85) {
      ctx.fillStyle = 'rgba(82,200,133,0.9)';
      ctx.font = 'bold 11px monospace';
      ctx.textAlign = 'center';
      ctx.fillText('arrived', W * 0.22, arrivedLabelY);
      ctx.textAlign = 'left';
    }
    ...
    ctx.textAlign = 'center';
    ctx.fillText('water', boxX, boxY - 6);
    ctx.textAlign = 'left';
    ...
    /* --- Energy pie chart --- */
    /* heatFraction: energy converted to heat so far */
    var heatFraction = cycle;
    var mechFraction = 1 - cycle;
    var startAngle = -Math.PI / 2;

    /* Mechanical energy slice (blue) */
    if (mechFraction > 0.001) {
      ctx.beginPath();
      ctx.moveTo(pieCX, pieCY);
      ctx.arc(pieCX, pieCY, pieR, startAngle, startAngle + mechFraction * Math.PI * 2);
      ctx.closePath();
    ...
    /* Heat energy slice (orange-red) */
    if (heatFraction > 0.001) {
      ctx.beginPath();
      ctx.moveTo(pieCX, pieCY);
      ctx.arc(pieCX, pieCY, pieR,
        startAngle + mechFraction * Math.PI * 2,
        startAngle + Math.PI * 2);
      ctx.closePath();
      ctx.fillStyle = 'rgba(200,120,60,0.75)';
      ctx.fill();
      ctx.strokeStyle = 'rgba(200,120,60,0.95)';
      ctx.lineWidth = 1.5;
      ctx.stroke();
    }
    ...
    ctx.fillText('mech', pieCX - pieR * 0.4, pieCY + pieR + 10);
    ctx.fillStyle = 'rgba(200,120,60,0.85)';
    ctx.fillText('heat', pieCX + pieR * 0.05, pieCY + pieR + 10);
    ctx.textAlign = 'left';

    /* Energy readout — positioned BELOW pie chart (readY = pieCY + pieR + 20) */
    ctx.fillStyle = COLOR;
    ctx.font = '13px monospace';
    ctx.textAlign = 'center';
    ctx.fillText('T = ' + temp.toFixed(1) + ' \u00b0C', pieCX, readY + 18);
    ctx.textAlign = 'left';
